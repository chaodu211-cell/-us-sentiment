...
| ● 蓝点 | 蓝点预警 | 快口径温度 < 25 且 VIX ≥ 30，当日触发 |
...
页面顶部的**综合温度**（六项等权）负责*描述*市场状态；红点用单独加权的**减仓温度**负责*标顶*：

```
减仓温度 = TOP2抱团×2 + 杠杆温度×2 + 上涨拥挤度×3 + 站上MA20×1 + 换手率×1   （权重归一）
```

拆开是因为实测标顶和标底靠的不是同一批因子：判别力最强的上涨拥挤度（秩相关 −0.348，
是第二名的两倍多）原先不在综合温度里，而上涨占比与站上MA20 相关 0.84、是同一个东西量两遍
却占掉温度方差的 38%。
